...
3. **Always validate output via script**:
   - run:
     ```bash
     python3 .claude/command-assets/app-planner/scripts/validate-and-generate.py validate --input <output-path> --mode <new|incremental>
     ```
...
1. **Preferred**: run the `/dev-pipeline-launcher` command (natural-language handoff)
