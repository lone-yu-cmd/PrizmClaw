...
The `.claude/commands/app-planner` placeholder represents the absolute path to this skill's directory. It is automatically resolved by the environment when the skill is invoked.
...
```bash
python3 .claude/commands/app-planner/scripts/validate-and-generate.py --input <path-to-written-file> --output <final-output-path>
```
...
```bash
python3 .claude/commands/app-planner/scripts/validate-and-generate.py validate --input feature-list.json
```
