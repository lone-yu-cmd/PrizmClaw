...
- **Session ID**: F-003-20260312031200
- **Feature ID**: F-003
- **Feature Title**: Feature and Bug Plan Ingestion
- **Feature Slug**: 003-feature-and-bug-plan-ingestion
...
You are the **session orchestrator**. Implement Feature F-003: "Feature and Bug Plan Ingestion".
...
实现 Telegram 侧计划文件接入能力，支持 feature-list.json 与 bug-fix-list.json 的上传、引用与落盘管理。系统需完成 JSON 解析、schema 校验、错误定位、版本化存储与回滚选择，使用户能在 Telegram 中完成“提供计划输入”的闭环。

该功能需兼容 app-planner/bug-planner 产物格式，并为后续 pipeline 执行提供稳定输入路径。

### Acceptance Criteria

- 支持通过 Telegram 上传或引用计划文件并成功保存到约定目录
- 对 feature-list 与 bug-fix-list 执行 schema 校验，失败时返回字段级错误
- 支持同名文件版本化管理，并可指定某个版本作为当前执行输入
- 当输入文件不合法时，不会污染当前可运行配置且会保留原有稳定版本
- 保存成功后可被 pipeline 控制层直接读取并启动执行
...
**ALWAYS** use per-feature subdirectory `.prizmkit/specs/003-feature-and-bug-plan-ingestion/`:

```
.prizmkit/specs/003-feature-and-bug-plan-ingestion/spec.md
.prizmkit/specs/003-feature-and-bug-plan-ingestion/plan.md
.prizmkit/specs/003-feature-and-bug-plan-ingestion/tasks.md
...
2. **If no reusable team exists, create a new one**:
   ...
   - Call `TeamCreate` with `team_name="prizm-dev-team-F-003"` and `description="Implementing Feature and Bug Plan Ingestion"`
...
```bash
python3 /Users/loneyu/SelfProjects/PrizmClaw/dev-pipeline/scripts/init-dev-team.py --project-root /Users/loneyu/SelfProjects/PrizmClaw --feature-id F-003 --feature-slug 003-feature-and-bug-plan-ingestion
```
...
- Spawn PM agent (Task tool, subagent_type="prizm-dev-team-pm", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-pm.md. For feature F-003 (slug: 003-feature-and-bug-plan-ingestion), complete all three planning steps in this single session:
  1. Run prizmkit-specify → generate `.prizmkit/specs/003-feature-and-bug-plan-ingestion/spec.md`. If there are `[NEEDS CLARIFICATION]` markers, run prizmkit-clarify to resolve them.
  2. Run prizmkit-plan → generate `.prizmkit/specs/003-feature-and-bug-plan-ingestion/plan.md` (architecture, components, interface design, data model, testing strategy, risk assessment — all in one file)
  3. Run prizmkit-tasks → generate `.prizmkit/specs/003-feature-and-bug-plan-ingestion/tasks.md` with `[ ]` checkboxes
  All three files go under `.prizmkit/specs/003-feature-and-bug-plan-ingestion/`."
...
- Spawn Reviewer agent (Task tool, subagent_type="prizm-dev-team-reviewer", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-reviewer.md. Run prizmkit-analyze for feature F-003 (slug: 003-feature-and-bug-plan-ingestion). Cross-check `.prizmkit/specs/003-feature-and-bug-plan-ingestion/spec.md`, `plan.md`, and `tasks.md` for consistency. Report any CRITICAL or HIGH issues."
...
- Read tasks from `.prizmkit/specs/003-feature-and-bug-plan-ingestion/tasks.md`
...
- Spawn Dev agent (Task tool, subagent_type="prizm-dev-team-dev", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-dev.md. Implement all tasks for feature F-003 (slug: 003-feature-and-bug-plan-ingestion) using prizmkit-implement with TDD. Read the plan from `.prizmkit/specs/003-feature-and-bug-plan-ingestion/plan.md` and tasks from `tasks.md`. Mark completed tasks [x] in tasks.md."
- **Wait for Dev to return**
- All tasks marked `[x]`, tests pass

#### Phase 6: Review
- Spawn Reviewer agent (Task tool, subagent_type="prizm-dev-team-reviewer", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-reviewer.md. For feature F-003 (slug: 003-feature-and-bug-plan-ingestion):
...
**7b.** Run `prizmkit.committer` (invoke the prizmkit-committer skill) → `feat(F-003): Feature and Bug Plan Ingestion`, do NOT push

### Step 3: Report Session Status

**CRITICAL**: Before this session ends, you MUST write the session status file.

Write to: `/Users/loneyu/SelfProjects/PrizmClaw/dev-pipeline/state/features/F-003/sessions/F-003-20260312031200/session-status.json`

```json
{
  "session_id": "F-003-20260312031200",
  "feature_id": "F-003",
  "feature_slug": "003-feature-and-bug-plan-ingestion",
  ...
  "artifacts": {
    "spec_path": ".prizmkit/specs/003-feature-and-bug-plan-ingestion/spec.md",
    "plan_path": ".prizmkit/specs/003-feature-and-bug-plan-ingestion/plan.md",
    "tasks_path": ".prizmkit/specs/003-feature-and-bug-plan-ingestion/tasks.md"
  },
...
| Feature Artifacts Dir | `.prizmkit/specs/003-feature-and-bug-plan-ingestion/` |
...
| Session Status Output | /Users/loneyu/SelfProjects/PrizmClaw/dev-pipeline/state/features/F-003/sessions/F-003-20260312031200/session-status.json |
...
- **All artifacts go under `.prizmkit/specs/003-feature-and-bug-plan-ingestion/`** — only 3 files: spec.md, plan.md, tasks.md
