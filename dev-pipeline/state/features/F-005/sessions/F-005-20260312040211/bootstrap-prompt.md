...
- **Session ID**: F-005-20260312040211
- **Feature ID**: F-005
- **Feature Title**: Pipeline Status and Log Streaming
- **Feature Slug**: 005-pipeline-status-and-log-streaming
...
You are the **session orchestrator**. Implement Feature F-005: "Pipeline Status and Log Streaming".
...
实现 pipeline 状态聚合与日志流式回传到 Telegram，包括当前执行 feature、已完成/失败统计、关键错误摘要、日志 tail 分页与节流推送。该功能应兼顾可读性与消息长度限制，避免刷屏并支持按需拉取详细日志。

### Acceptance Criteria

- `/status` 可返回当前 pipeline 阶段、统计信息和正在执行的特性
- `/logs` 支持读取最近日志片段并支持偏移或行数参数
- 长日志输出自动分段并遵守 Telegram 消息长度限制
- 执行中可按固定策略推送心跳进度，失败时自动推送错误摘要
- 日志读取异常或状态文件缺失时返回可诊断错误而非静默失败

### Dependencies (Already Completed)

- F-004 - Pipeline Process Controller (completed)
...
**ALWAYS** use per-feature subdirectory `.prizmkit/specs/005-pipeline-status-and-log-streaming/`:

```
.prizmkit/specs/005-pipeline-status-and-log-streaming/spec.md
.prizmkit/specs/005-pipeline-status-and-log-streaming/plan.md
.prizmkit/specs/005-pipeline-status-and-log-streaming/tasks.md
...
2. **If no reusable team exists, create a new one**:
   ...
   - Call `TeamCreate` with `team_name="prizm-dev-team-F-005"` and `description="Implementing Pipeline Status and Log Streaming"`
...
```bash
python3 /Users/loneyu/SelfProjects/PrizmClaw/dev-pipeline/scripts/init-dev-team.py --project-root /Users/loneyu/SelfProjects/PrizmClaw --feature-id F-005 --feature-slug 005-pipeline-status-and-log-streaming
```
...
- Spawn PM agent (Task tool, subagent_type="prizm-dev-team-pm", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-pm.md. For feature F-005 (slug: 005-pipeline-status-and-log-streaming), complete all three planning steps in this single session:
  1. Run prizmkit-specify → generate `.prizmkit/specs/005-pipeline-status-and-log-streaming/spec.md`. If there are `[NEEDS CLARIFICATION]` markers, run prizmkit-clarify to resolve them.
  2. Run prizmkit-plan → generate `.prizmkit/specs/005-pipeline-status-and-log-streaming/plan.md` (architecture, components, interface design, data model, testing strategy, risk assessment — all in one file)
  3. Run prizmkit-tasks → generate `.prizmkit/specs/005-pipeline-status-and-log-streaming/tasks.md` with `[ ]` checkboxes
  All three files go under `.prizmkit/specs/005-pipeline-status-and-log-streaming/`."
...
- Spawn Reviewer agent (Task tool, subagent_type="prizm-dev-team-reviewer", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-reviewer.md. Run prizmkit-analyze for feature F-005 (slug: 005-pipeline-status-and-log-streaming). Cross-check `.prizmkit/specs/005-pipeline-status-and-log-streaming/spec.md`, `plan.md`, and `tasks.md` for consistency. Report any CRITICAL or HIGH issues."
...
- Read tasks from `.prizmkit/specs/005-pipeline-status-and-log-streaming/tasks.md`
...
- Spawn Dev agent (Task tool, subagent_type="prizm-dev-team-dev", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-dev.md. Implement all tasks for feature F-005 (slug: 005-pipeline-status-and-log-streaming) using prizmkit-implement with TDD. Read the plan from `.prizmkit/specs/005-pipeline-status-and-log-streaming/plan.md` and tasks from `tasks.md`. Mark completed tasks [x] in tasks.md."
- **Wait for Dev to return**
- All tasks marked `[x]`, tests pass

#### Phase 6: Review
- Spawn Reviewer agent (Task tool, subagent_type="prizm-dev-team-reviewer", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-reviewer.md. For feature F-005 (slug: 005-pipeline-status-and-log-streaming):
...
**7b.** Run `prizmkit.committer` (invoke the prizmkit-committer skill) → `feat(F-005): Pipeline Status and Log Streaming`, do NOT push

### Step 3: Report Session Status

**CRITICAL**: Before this session ends, you MUST write the session status file.

Write to: `/Users/loneyu/SelfProjects/PrizmClaw/dev-pipeline/state/features/F-005/sessions/F-005-20260312040211/session-status.json`

```json
{
  "session_id": "F-005-20260312040211",
  "feature_id": "F-005",
  "feature_slug": "005-pipeline-status-and-log-streaming",
  ...
  "artifacts": {
    "spec_path": ".prizmkit/specs/005-pipeline-status-and-log-streaming/spec.md",
    "plan_path": ".prizmkit/specs/005-pipeline-status-and-log-streaming/plan.md",
    "tasks_path": ".prizmkit/specs/005-pipeline-status-and-log-streaming/tasks.md"
  },
...
| Feature Artifacts Dir | `.prizmkit/specs/005-pipeline-status-and-log-streaming/` |
...
| Session Status Output | /Users/loneyu/SelfProjects/PrizmClaw/dev-pipeline/state/features/F-005/sessions/F-005-20260312040211/session-status.json |
...
- **All artifacts go under `.prizmkit/specs/005-pipeline-status-and-log-streaming/`** — only 3 files: spec.md, plan.md, tasks.md
