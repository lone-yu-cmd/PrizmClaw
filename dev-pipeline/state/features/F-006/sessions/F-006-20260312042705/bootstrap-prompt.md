...
- **Session ID**: F-006-20260312042705
- **Feature ID**: F-006
- **Feature Title**: Safety and Permission Guard
- **Feature Slug**: 006-safety-and-permission-guard
...
You are the **session orchestrator**. Implement Feature F-006: "Safety and Permission Guard".
...
为 Telegram 管道控制增加安全护栏：命令白名单、用户权限分级、参数净化、敏感动作二次确认与审计日志。重点覆盖 stop/reset/retry/exec 等高风险指令，确保“可用”与“可控”并存。

### Acceptance Criteria

- 支持基于用户 ID 的权限分级与高风险命令授权校验
- 高风险命令执行前需要明确确认或满足策略条件
- 命令参数经过净化与规则校验，拒绝非法路径与危险参数组合
- 每次敏感操作都会写入审计日志并可按会话检索
...
**ALWAYS** use per-feature subdirectory `.prizmkit/specs/006-safety-and-permission-guard/`:

```
.prizmkit/specs/006-safety-and-permission-guard/spec.md
.prizmkit/specs/006-safety-and-permission-guard/plan.md
.prizmkit/specs/006-safety-and-permission-guard/tasks.md
...
2. **If no reusable team exists, create a new one**:
   ...
   - Call `TeamCreate` with `team_name="prizm-dev-team-F-006"` and `description="Implementing Safety and Permission Guard"`
...
```bash
python3 /Users/loneyu/SelfProjects/PrizmClaw/dev-pipeline/scripts/init-dev-team.py --project-root /Users/loneyu/SelfProjects/PrizmClaw --feature-id F-006 --feature-slug 006-safety-and-permission-guard
```
...
- Spawn PM agent (Task tool, subagent_type="prizm-dev-team-pm", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-pm.md. For feature F-006 (slug: 006-safety-and-permission-guard), complete all three planning steps in this single session:
  1. Run prizmkit-specify → generate `.prizmkit/specs/006-safety-and-permission-guard/spec.md` (concise, under 150 lines)
  2. Run prizmkit-plan → generate `.prizmkit/specs/006-safety-and-permission-guard/plan.md` (architecture, components, interface design, data model, testing strategy — all in one file)
  3. Run prizmkit-tasks → generate `.prizmkit/specs/006-safety-and-permission-guard/tasks.md` with `[ ]` checkboxes
  All three files go under `.prizmkit/specs/006-safety-and-permission-guard/`."
...
- Spawn Reviewer agent (Task tool, subagent_type="prizm-dev-team-reviewer", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-reviewer.md. Run prizmkit-analyze for feature F-006 (slug: 006-safety-and-permission-guard). Cross-check `.prizmkit/specs/006-safety-and-permission-guard/spec.md`, `plan.md`, and `tasks.md` for consistency. Report any CRITICAL or HIGH issues."
...
- Read tasks from `.prizmkit/specs/006-safety-and-permission-guard/tasks.md`
...
- Spawn Dev agent (Task tool, subagent_type="prizm-dev-team-dev", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-dev.md. Implement all tasks for feature F-006 (slug: 006-safety-and-permission-guard) using prizmkit-implement with TDD. Read the plan from `.prizmkit/specs/006-safety-and-permission-guard/plan.md` and tasks from `tasks.md`. Mark completed tasks [x] in tasks.md."
- **Wait for Dev to return**
- All tasks marked `[x]`, tests pass

#### Phase 6: Review
- Spawn Reviewer agent (Task tool, subagent_type="prizm-dev-team-reviewer", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-reviewer.md. For feature F-006 (slug: 006-safety-and-permission-guard):
...
**7b.** Run `prizmkit.committer` (invoke the prizmkit-committer skill) → `feat(F-006): Safety and Permission Guard`, do NOT push

### Step 3: Report Session Status

**CRITICAL**: Before this session ends, you MUST write the session status file.

Write to: `/Users/loneyu/SelfProjects/PrizmClaw/dev-pipeline/state/features/F-006/sessions/F-006-20260312042705/session-status.json`

```json
{
  "session_id": "F-006-20260312042705",
  "feature_id": "F-006",
  "feature_slug": "006-safety-and-permission-guard",
  ...
  "artifacts": {
    "spec_path": ".prizmkit/specs/006-safety-and-permission-guard/spec.md",
    "plan_path": ".prizmkit/specs/006-safety-and-permission-guard/plan.md",
    "tasks_path": ".prizmkit/specs/006-safety-and-permission-guard/tasks.md"
  },
...
| Feature Artifacts Dir | `.prizmkit/specs/006-safety-and-permission-guard/` |
...
| Session Status Output | /Users/loneyu/SelfProjects/PrizmClaw/dev-pipeline/state/features/F-006/sessions/F-006-20260312042705/session-status.json |
...
- **All artifacts go under `.prizmkit/specs/006-safety-and-permission-guard/`** — only 3 files: spec.md, plan.md, tasks.md
