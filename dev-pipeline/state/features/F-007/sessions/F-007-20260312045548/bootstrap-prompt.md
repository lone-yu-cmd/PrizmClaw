...
- **Session ID**: F-007-20260312045548
- **Feature ID**: F-007
- **Feature Title**: Test and Validation Suite
- **Feature Slug**: 007-test-and-validation-suite
...
You are the **session orchestrator**. Implement Feature F-007: "Test and Validation Suite".
...
建立 Telegram 管道能力的测试体系，覆盖命令路由、计划文件接入、进程控制、状态聚合与权限校验。测试策略以 Jest 为主，包含单元与必要的集成测试，确保核心流程在改动后可回归验证。

### Acceptance Criteria

- Jest 测试可覆盖命令解析、控制层返回与错误路径
- 提供至少一条端到端级集成测试验证 start → status → stop 主链路
- 计划文件校验与版本切换存在自动化测试覆盖
- 权限与安全策略存在负向测试，确保未授权操作被阻断

### Dependencies (Already Completed)

- F-003 - Feature and Bug Plan Ingestion (completed)
...
**ALWAYS** use per-feature subdirectory `.prizmkit/specs/007-test-and-validation-suite/`:

```
.prizmkit/specs/007-test-and-validation-suite/spec.md
.prizmkit/specs/007-test-and-validation-suite/plan.md
.prizmkit/specs/007-test-and-validation-suite/tasks.md
...
2. **If no reusable team exists, create a new one**:
   ...
   - Call `TeamCreate` with `team_name="prizm-dev-team-F-007"` and `description="Implementing Test and Validation Suite"`
...
```bash
python3 /Users/loneyu/SelfProjects/PrizmClaw/dev-pipeline/scripts/init-dev-team.py --project-root /Users/loneyu/SelfProjects/PrizmClaw --feature-id F-007 --feature-slug 007-test-and-validation-suite
```
...
- Spawn PM agent (Task tool, subagent_type="prizm-dev-team-pm", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-pm.md. For feature F-007 (slug: 007-test-and-validation-suite), complete all three planning steps in this single session:
  1. Run prizmkit-specify → generate `.prizmkit/specs/007-test-and-validation-suite/spec.md` (concise, under 150 lines)
  2. Run prizmkit-plan → generate `.prizmkit/specs/007-test-and-validation-suite/plan.md` (architecture, components, interface design, data model, testing strategy — all in one file)
  3. Run prizmkit-tasks → generate `.prizmkit/specs/007-test-and-validation-suite/tasks.md` with `[ ]` checkboxes
  All three files go under `.prizmkit/specs/007-test-and-validation-suite/`."
...
- Spawn Reviewer agent (Task tool, subagent_type="prizm-dev-team-reviewer", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-reviewer.md. Run prizmkit-analyze for feature F-007 (slug: 007-test-and-validation-suite). Cross-check `.prizmkit/specs/007-test-and-validation-suite/spec.md`, `plan.md`, and `tasks.md` for consistency. Report any CRITICAL or HIGH issues."
...
- Read tasks from `.prizmkit/specs/007-test-and-validation-suite/tasks.md`
...
- Spawn Dev agent (Task tool, subagent_type="prizm-dev-team-dev", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-dev.md. Implement all tasks for feature F-007 (slug: 007-test-and-validation-suite) using prizmkit-implement with TDD. Read the plan from `.prizmkit/specs/007-test-and-validation-suite/plan.md` and tasks from `tasks.md`. Mark completed tasks [x] in tasks.md."
- **Wait for Dev to return**
- All tasks marked `[x]`, tests pass

#### Phase 6: Review
- Spawn Reviewer agent (Task tool, subagent_type="prizm-dev-team-reviewer", run_in_background=false)
  Prompt: "Read /Users/loneyu/SelfProjects/PrizmClaw/.codebuddy/agents/prizm-dev-team-reviewer.md. For feature F-007 (slug: 007-test-and-validation-suite):
...
**7b.** Run `prizmkit.committer` (invoke the prizmkit-committer skill) → `feat(F-007): Test and Validation Suite`, do NOT push

### Step 3: Report Session Status

**CRITICAL**: Before this session ends, you MUST write the session status file.

Write to: `/Users/loneyu/SelfProjects/PrizmClaw/dev-pipeline/state/features/F-007/sessions/F-007-20260312045548/session-status.json`

```json
{
  "session_id": "F-007-20260312045548",
  "feature_id": "F-007",
  "feature_slug": "007-test-and-validation-suite",
  ...
  "artifacts": {
    "spec_path": ".prizmkit/specs/007-test-and-validation-suite/spec.md",
    "plan_path": ".prizmkit/specs/007-test-and-validation-suite/plan.md",
    "tasks_path": ".prizmkit/specs/007-test-and-validation-suite/tasks.md"
  },
...
| Feature Artifacts Dir | `.prizmkit/specs/007-test-and-validation-suite/` |
...
| Session Status Output | /Users/loneyu/SelfProjects/PrizmClaw/dev-pipeline/state/features/F-007/sessions/F-007-20260312045548/session-status.json |
...
- **All artifacts go under `.prizmkit/specs/007-test-and-validation-suite/`** — only 3 files: spec.md, plan.md, tasks.md
