...
- **Feature ID**: F-017 | **Session**: F-017-20260322184252 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-017: "Runtime Config Manager".
...
手机端运行时配置查看与修改能力。用户通过 /config 命令在 Telegram 中查看和动态修改 Bot 运行时配置，无需编辑 .env 文件或重启服务。

命令设计：/config 列出所有当前生效的配置（敏感字段如 token 脱敏显示）；/config get <KEY> 查看单个配置值；/config set <KEY>=<VALUE> 动态修改配置值（仅允许白名单中的安全配置项，如 LOG_LEVEL、REQUEST_TIMEOUT_MS、AI_CLI_HEARTBEAT_MS 等）；/config reset <KEY> 重置为 .env 文件中的原始值。

安全限制：token、密钥等敏感配置不可通过 /config set 修改；配置修改需 admin 权限（集成 F-006）；每次修改写入审计日志。

配置热加载：修改后立即生效，不需要重启 Bot 进程。
...
```
.prizmkit/specs/017-runtime-config-manager/context-snapshot.md
.prizmkit/specs/017-runtime-config-manager/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/017-runtime-config-manager/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/017-runtime-config-manager/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/017-runtime-config-manager/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-017): Runtime Config Manager`, do NOT push
...
git commit -m "chore(F-017): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/017-runtime-config-manager/` |
| Context Snapshot | `.prizmkit/specs/017-runtime-config-manager/context-snapshot.md` |
