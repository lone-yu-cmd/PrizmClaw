...
- **Feature ID**: F-022 | **Session**: F-022-20260322230557 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-022: "Web Chat Input Placeholder Update".
...
将 Web 管理台聊天输入框的 placeholder 文本更新为「请输入对话」，提升界面可读性和用户引导性。修改范围限于 public/index.html 中 chatInput textarea 元素的 placeholder 属性，无后端变更。
...
```
.prizmkit/specs/022-web-chat-input-placeholder-update/context-snapshot.md
.prizmkit/specs/022-web-chat-input-placeholder-update/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/022-web-chat-input-placeholder-update/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/022-web-chat-input-placeholder-update/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/022-web-chat-input-placeholder-update/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-022): Web Chat Input Placeholder Update`, do NOT push
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/022-web-chat-input-placeholder-update/` |
| Context Snapshot | `.prizmkit/specs/022-web-chat-input-placeholder-update/context-snapshot.md` |
