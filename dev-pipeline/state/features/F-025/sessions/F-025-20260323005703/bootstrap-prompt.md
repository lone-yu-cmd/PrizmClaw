...
- **Feature ID**: F-025 | **Session**: F-025-20260323005703 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-025: "Web Header Sticky Behavior".
...
为 Web 管理台页面头部（.app-header）添加滚动时的粘性定位行为。在 public/styles.css 中为 .app-header 添加 position: sticky 和滚动时的视觉分隔线，确保长页面操作时标题区域始终可见。无后端变更。

### Acceptance Criteria

- Given 页面内容超出视口高度，When 用户向下滚动，Then 页面头部固定在顶部持续可见
- Given 头部处于 sticky 状态，When 用户向上滚动至顶部，Then 头部恢复正常文档流位置
- Given 头部固定时，When 渲染头部下边框，Then 显示分隔线以区分头部与内容区
...
```
.prizmkit/specs/025-web-header-sticky-behavior/context-snapshot.md
.prizmkit/specs/025-web-header-sticky-behavior/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/025-web-header-sticky-behavior/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/025-web-header-sticky-behavior/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/025-web-header-sticky-behavior/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-025): Web Header Sticky Behavior`, do NOT push
...
git commit -m "chore(F-025): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/025-web-header-sticky-behavior/` |
| Context Snapshot | `.prizmkit/specs/025-web-header-sticky-behavior/context-snapshot.md` |
