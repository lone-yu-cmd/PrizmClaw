...
- **Feature ID**: F-026 | **Session**: F-026-20260323012530 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-026: "Web Status Indicator Color States".
...
为 Web 管理台状态指示器（#status）添加多种颜色状态（就绪/连接中/错误）的视觉区分。在 public/styles.css 中新增 .status.connected、.status.error 样式类，通过不同背景色和文字色传达连接状态。
...
```
.prizmkit/specs/026-web-status-indicator-color-states/context-snapshot.md
.prizmkit/specs/026-web-status-indicator-color-states/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/026-web-status-indicator-color-states/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/026-web-status-indicator-color-states/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/026-web-status-indicator-color-states/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-026): Web Status Indicator Color States`, do NOT push
...
git commit -m "chore(F-026): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/026-web-status-indicator-color-states/` |
| Context Snapshot | `.prizmkit/specs/026-web-status-indicator-color-states/context-snapshot.md` |
