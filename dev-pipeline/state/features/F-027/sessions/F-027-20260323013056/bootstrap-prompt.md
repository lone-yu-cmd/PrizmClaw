...
- **Feature ID**: F-027 | **Session**: F-027-20260323013056 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-027: "Web Chat Message Fade-In Animation".
...
为 Web 管理台聊天消息列表中新增消息添加淡入动画。在 public/styles.css 中为 .message 添加 @keyframes fade-in 动画，使新消息出现时有平滑过渡效果，提升对话界面的流畅感。无后端变更。

### Acceptance Criteria

- Given 用户发送消息或收到 AI 回复，When 新消息被插入到聊天列表，Then 消息以淡入动画出现
- Given 消息淡入动画，When 动画完成，Then 消息以完全不透明状态静止显示
- Given 页面中已有历史消息，When 页面首次加载，Then 已有消息不触发淡入动画
...
```
.prizmkit/specs/027-web-chat-message-fade-in-animation/context-snapshot.md
.prizmkit/specs/027-web-chat-message-fade-in-animation/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/027-web-chat-message-fade-in-animation/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/027-web-chat-message-fade-in-animation/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/027-web-chat-message-fade-in-animation/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-027): Web Chat Message Fade-In Animation`, do NOT push
...
git commit -m "chore(F-027): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/027-web-chat-message-fade-in-animation/` |
| Context Snapshot | `.prizmkit/specs/027-web-chat-message-fade-in-animation/context-snapshot.md` |
