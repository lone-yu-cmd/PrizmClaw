...
- **Feature ID**: F-028 | **Session**: F-028-20260323081051 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-028: "Web Input Character Counter".
...
为 Web 管理台聊天输入框（chatInput）添加字符计数显示。在 public/index.html 和 public/styles.css 中新增字符计数元素，接近字符上限时以警告色高亮提示，帮助用户感知输入长度。无后端变更。

### Acceptance Criteria

- Given 用户在聊天输入框中输入内容，When 字符数变化，Then 计数器实时显示当前字符数
- Given 字符数超过上限的 80%，When 渲染计数器，Then 计数器以警告色（--warning）高亮显示
- Given 输入框为空，When 渲染计数器，Then 计数器显示 0 且为默认样式

### Dependencies (Already Completed)

- F-022 - Web Input Focus Shadow Effect (completed)
...
```
.prizmkit/specs/028-web-input-character-counter/context-snapshot.md
.prizmkit/specs/028-web-input-character-counter/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/028-web-input-character-counter/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/028-web-input-character-counter/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/028-web-input-character-counter/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-028): Web Input Character Counter`, do NOT push
...
git commit -m "chore(F-028): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/028-web-input-character-counter/` |
| Context Snapshot | `.prizmkit/specs/028-web-input-character-counter/context-snapshot.md` |
