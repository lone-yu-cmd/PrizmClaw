...
- **Feature ID**: F-029 | **Session**: F-029-20260323081920 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-029: "Web Output Box Syntax Highlight".
...
为 Web 管理台 stdout/stderr 输出框（.output-box）添加基础语法高亮样式。在 public/styles.css 中为 .output-box 内的关键输出模式（错误行、路径、数字）添加颜色区分，提升命令输出的可读性。无后端变更。
...
- F-019 - Web Chat Command Support (completed)
...
```
.prizmkit/specs/029-web-output-box-syntax-highlight/context-snapshot.md
.prizmkit/specs/029-web-output-box-syntax-highlight/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/029-web-output-box-syntax-highlight/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/029-web-output-box-syntax-highlight/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/029-web-output-box-syntax-highlight/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-029): Web Output Box Syntax Highlight`, do NOT push
...
git commit -m "chore(F-029): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/029-web-output-box-syntax-highlight/` |
| Context Snapshot | `.prizmkit/specs/029-web-output-box-syntax-highlight/context-snapshot.md` |
