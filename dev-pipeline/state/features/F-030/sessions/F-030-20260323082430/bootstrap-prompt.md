...
- **Feature ID**: F-030 | **Session**: F-030-20260323082430 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-030: "Web Responsive Mobile Layout".
...
优化 Web 管理台在移动端（<600px）的布局适配。在 public/styles.css 中补充更小断点的媒体查询，调整 header、chat-form、panel padding 等元素在手机屏幕下的显示方式，确保移动端可用性。无后端变更。

### Acceptance Criteria

- Given 屏幕宽度小于 600px，When 渲染页面，Then 所有 panel 以单列布局排列
- Given 移动端，When 渲染聊天输入区域，Then 发送按钮与输入框垂直排列不溢出屏幕
- Given 移动端，When 渲染页面头部，Then 标题与状态指示器不发生文字截断或重叠
...
```
.prizmkit/specs/030-web-responsive-mobile-layout/context-snapshot.md
.prizmkit/specs/030-web-responsive-mobile-layout/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/030-web-responsive-mobile-layout/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/030-web-responsive-mobile-layout/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/030-web-responsive-mobile-layout/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-030): Web Responsive Mobile Layout`, do NOT push
...
git commit -m "chore(F-030): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/030-web-responsive-mobile-layout/` |
| Context Snapshot | `.prizmkit/specs/030-web-responsive-mobile-layout/context-snapshot.md` |
