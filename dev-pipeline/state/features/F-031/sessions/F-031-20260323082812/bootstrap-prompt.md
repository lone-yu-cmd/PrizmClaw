...
- **Feature ID**: F-031 | **Session**: F-031-20260323082812 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-031: "Web Dark Mode Support".
...
为 Web 管理台添加深色模式支持。在 public/styles.css 中通过 @media (prefers-color-scheme: dark) 覆盖 CSS 变量，使界面跟随操作系统深色主题自动切换，不需要额外的切换按钮。无后端变更。

### Acceptance Criteria

- Given 操作系统切换为深色模式，When 加载或刷新 Web 管理台，Then 界面自动切换为深色配色方案
- Given 深色模式，When 渲染所有 panel、input、button，Then 各组件颜色对比度符合可读性要求
- Given 操作系统切换回浅色模式，When 加载或刷新页面，Then 界面恢复浅色配色方案
...
```
.prizmkit/specs/031-web-dark-mode-support/context-snapshot.md
.prizmkit/specs/031-web-dark-mode-support/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/031-web-dark-mode-support/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/031-web-dark-mode-support/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/031-web-dark-mode-support/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-031): Web Dark Mode Support`, do NOT push
...
git commit -m "chore(F-031): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/031-web-dark-mode-support/` |
| Context Snapshot | `.prizmkit/specs/031-web-dark-mode-support/context-snapshot.md` |
