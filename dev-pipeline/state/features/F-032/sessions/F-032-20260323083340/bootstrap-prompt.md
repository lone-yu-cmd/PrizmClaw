...
- **Feature ID**: F-032 | **Session**: F-032-20260323083340 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-032: "Web Screenshot Preview Lightbox".
...
为 Web 管理台截图预览图片（#screenshotImage）添加点击放大的 lightbox 效果。在 public/styles.css 和 public/index.html 中实现纯 CSS 或轻量 JS 的模态预览，用户点击截图后可查看原始尺寸图片。无后端变更。
...
```
.prizmkit/specs/032-web-screenshot-preview-lightbox/context-snapshot.md
.prizmkit/specs/032-web-screenshot-preview-lightbox/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/032-web-screenshot-preview-lightbox/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/032-web-screenshot-preview-lightbox/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/032-web-screenshot-preview-lightbox/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-032): Web Screenshot Preview Lightbox`, do NOT push
...
git commit -m "chore(F-032): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/032-web-screenshot-preview-lightbox/` |
| Context Snapshot | `.prizmkit/specs/032-web-screenshot-preview-lightbox/context-snapshot.md` |
