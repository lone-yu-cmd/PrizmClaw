...
- **Feature ID**: F-033 | **Session**: F-033-20260323084146 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-033: "Web Toast Notification System".
...
为 Web 管理台添加轻量 Toast 通知组件，用于操作成功/失败的临时提示。在 public/styles.css 和 public/index.html 中实现固定定位的 toast 容器，支持 success/error/info 三种类型，3 秒后自动消失。无后端变更。
...
```
.prizmkit/specs/033-web-toast-notification-system/context-snapshot.md
.prizmkit/specs/033-web-toast-notification-system/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/033-web-toast-notification-system/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/033-web-toast-notification-system/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/033-web-toast-notification-system/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-033): Web Toast Notification System`, do NOT push
...
git commit -m "chore(F-033): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/033-web-toast-notification-system/` |
| Context Snapshot | `.prizmkit/specs/033-web-toast-notification-system/context-snapshot.md` |
