...
- **Feature ID**: F-035 | **Session**: F-035-20260323085455 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-035: "Web Keyboard Shortcut Hints".
...
在 Web 管理台关键交互元素上显示键盘快捷键提示标签。在 public/styles.css 中为 .kbd 组件添加样式，在发送按钮旁显示 Ctrl+Enter 提示，在命令输入框旁显示 Enter 提示。无后端变更。
...
```
.prizmkit/specs/035-web-keyboard-shortcut-hints/context-snapshot.md
.prizmkit/specs/035-web-keyboard-shortcut-hints/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/035-web-keyboard-shortcut-hints/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/035-web-keyboard-shortcut-hints/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/035-web-keyboard-shortcut-hints/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-035): Web Keyboard Shortcut Hints`, do NOT push
...
git commit -m "chore(F-035): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/035-web-keyboard-shortcut-hints/` |
| Context Snapshot | `.prizmkit/specs/035-web-keyboard-shortcut-hints/context-snapshot.md` |
