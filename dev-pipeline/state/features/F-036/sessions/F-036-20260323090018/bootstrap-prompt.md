...
- **Feature ID**: F-036 | **Session**: F-036-20260323090018 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-036: "Web Empty State Illustrations".
...
为 Web 管理台聊天消息区和命令输出区的空状态添加引导文案和图标。在 public/styles.css 和 public/index.html 中实现 .empty-state 组件，当列表为空时居中展示提示图标与说明文字。无后端变更。

### Acceptance Criteria

- Given 聊天消息区无消息，When 渲染页面，Then 显示空状态提示（图标+文字），引导用户发送第一条消息
- Given 命令输出区无内容，When 渲染页面，Then 显示空状态提示，引导用户执行命令
- Given 用户发送消息或执行命令后，When 内容出现，Then 空状态提示自动隐藏
...
```
.prizmkit/specs/036-web-empty-state-illustrations/context-snapshot.md
.prizmkit/specs/036-web-empty-state-illustrations/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/036-web-empty-state-illustrations/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/036-web-empty-state-illustrations/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/036-web-empty-state-illustrations/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-036): Web Empty State Illustrations`, do NOT push
...
git commit -m "chore(F-036): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/036-web-empty-state-illustrations/` |
| Context Snapshot | `.prizmkit/specs/036-web-empty-state-illustrations/context-snapshot.md` |
