...
- **Feature ID**: F-037 | **Session**: F-037-20260323090645 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-037: "Web Scroll To Bottom Button".
...
为 Web 管理台聊天消息区添加「滚动到底部」浮动按钮。在 public/styles.css 和 public/index.html 中实现当用户向上滚动时出现的固定定位按钮，点击后平滑滚动至最新消息。无后端变更。
...
```
.prizmkit/specs/037-web-scroll-to-bottom-button/context-snapshot.md
.prizmkit/specs/037-web-scroll-to-bottom-button/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/037-web-scroll-to-bottom-button/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/037-web-scroll-to-bottom-button/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/037-web-scroll-to-bottom-button/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-037): Web Scroll To Bottom Button`, do NOT push
...
git commit -m "chore(F-037): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/037-web-scroll-to-bottom-button/` |
| Context Snapshot | `.prizmkit/specs/037-web-scroll-to-bottom-button/context-snapshot.md` |
