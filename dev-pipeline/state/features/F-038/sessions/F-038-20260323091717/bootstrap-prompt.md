...
- **Feature ID**: F-038 | **Session**: F-038-20260323091717 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-038: "Web Copy To Clipboard Button".
...
为 Web 管理台所有 .output-box 输出框添加一键复制按钮。在 public/styles.css 和 public/index.html 中为每个输出框右上角添加复制图标按钮，点击后将框内文本写入剪贴板并以 toast 提示反馈。无后端变更。
...
- F-033 - Web Toast Notification System (completed)
...
```
.prizmkit/specs/038-web-copy-to-clipboard-button/context-snapshot.md
.prizmkit/specs/038-web-copy-to-clipboard-button/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/038-web-copy-to-clipboard-button/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/038-web-copy-to-clipboard-button/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/038-web-copy-to-clipboard-button/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-038): Web Copy To Clipboard Button`, do NOT push
...
git commit -m "chore(F-038): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/038-web-copy-to-clipboard-button/` |
| Context Snapshot | `.prizmkit/specs/038-web-copy-to-clipboard-button/context-snapshot.md` |
