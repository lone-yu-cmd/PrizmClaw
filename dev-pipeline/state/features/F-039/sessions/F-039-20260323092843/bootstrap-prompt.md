...
- **Feature ID**: F-039 | **Session**: F-039-20260323092843 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-039: "Web Session Info Display Panel".
...
在 Web 管理台连接设置区域下方添加当前会话信息展示面板，显示 Session ID、连接状态、已绑定 Telegram Chat ID 等运行时信息。在 public/styles.css 和 public/index.html 中实现只读信息卡片组件。无后端变更。
...
- F-026 - Web Status Indicator Color States (completed)
...
```
.prizmkit/specs/039-web-session-info-display-panel/context-snapshot.md
.prizmkit/specs/039-web-session-info-display-panel/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/039-web-session-info-display-panel/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/039-web-session-info-display-panel/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/039-web-session-info-display-panel/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-039): Web Session Info Display Panel`, do NOT push
...
git commit -m "chore(F-039): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/039-web-session-info-display-panel/` |
| Context Snapshot | `.prizmkit/specs/039-web-session-info-display-panel/context-snapshot.md` |
