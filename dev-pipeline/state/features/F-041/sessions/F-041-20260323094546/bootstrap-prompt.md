...
- **Feature ID**: F-041 | **Session**: F-041-20260323094546 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-041: "Web Message Timestamp Display".
...
为 Web 管理台聊天消息添加发送时间戳显示。在 public/styles.css 和 public/js/main.js 中为每条消息附加本地时间标签（HH:MM 格式），以小字体显示在消息角落，提升对话可追溯性。无后端变更。

### Acceptance Criteria

- Given 用户发送消息，When 消息渲染到聊天区，Then 消息右下角显示发送时间（HH:MM 格式）
- Given AI 回复消息，When 消息渲染到聊天区，Then 消息右下角显示接收时间
- Given 时间戳文字，When 渲染，Then 以小号字体和 --muted 颜色显示，不干扰消息主体阅读

### Dependencies (Already Completed)

- F-027 - Web Chat Message Fade-In Animation (completed)
...
```
.prizmkit/specs/041-web-message-timestamp-display/context-snapshot.md
.prizmkit/specs/041-web-message-timestamp-display/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/041-web-message-timestamp-display/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/041-web-message-timestamp-display/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/041-web-message-timestamp-display/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-041): Web Message Timestamp Display`, do NOT push
...
git commit -m "chore(F-041): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/041-web-message-timestamp-display/` |
| Context Snapshot | `.prizmkit/specs/041-web-message-timestamp-display/context-snapshot.md` |
