...
- **Feature ID**: F-042 | **Session**: F-042-20260323101343 | **Run**: run-20260322-013453
- **Complexity**: low | **Retry**: 2 / 3
- **Previous Status**: N/A (first run) | **Resume From**: null
- **Init**: true | Artifacts: spec=false plan=false

## Your Mission

You are the **session orchestrator**. Implement Feature F-042: "Web Exec History Dropdown".
...
为 Web 管理台系统命令输入框添加历史命令下拉选择。在 public/styles.css 和 public/js/main.js 中实现最近 10 条命令的本地存储与下拉 UI，用户可快速重用历史命令。无后端变更。

### Acceptance Criteria

- Given 用户成功执行命令，When 命令完成，Then 该命令存入本地历史（最多 10 条，超出时移除最旧）
- Given 命令输入框有焦点，When 用户按向上方向键，Then 依次填入历史命令
- Given 命令历史存在，When 用户点击输入框旁的历史图标，Then 显示历史命令下拉列表可选择

### Dependencies (Already Completed)

- F-029 - Web Output Box Syntax Highlight (completed)
...
```
.prizmkit/specs/042-web-exec-history-dropdown/context-snapshot.md
.prizmkit/specs/042-web-exec-history-dropdown/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/042-web-exec-history-dropdown/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/042-web-exec-history-dropdown/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/042-web-exec-history-dropdown/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-042): Web Exec History Dropdown`, do NOT push
...
git commit -m "chore(F-042): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/042-web-exec-history-dropdown/` |
| Context Snapshot | `.prizmkit/specs/042-web-exec-history-dropdown/context-snapshot.md` |
