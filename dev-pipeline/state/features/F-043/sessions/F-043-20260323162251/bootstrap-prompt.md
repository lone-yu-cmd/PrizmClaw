...
- **Feature ID**: F-043 | **Session**: F-043-20260323162251 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-043: "Web Font Size Accessibility Toggle".
...
为 Web 管理台添加字体大小切换控件，支持小/中/大三档，满足不同视力用户的可读性需求。在 public/styles.css 中通过 CSS 变量 --font-scale 控制全局字体倍率，在 public/index.html 中添加切换按钮，选择结果持久化至 localStorage。无后端变更。

### Acceptance Criteria

- Given 页面加载完成，When 渲染字体切换控件，Then 显示小/中/大三个选项，默认选中中档
- Given 用户选择大字体档，When 切换生效，Then 页面所有文字按比例放大，布局不破损
- Given 用户刷新页面，When 页面重新加载，Then 上次选择的字体档位从 localStorage 恢复

### Dependencies (Already Completed)

- F-031 - Web Dark Mode Support (completed)
...
```
.prizmkit/specs/043-web-font-size-accessibility-toggle/context-snapshot.md
.prizmkit/specs/043-web-font-size-accessibility-toggle/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/043-web-font-size-accessibility-toggle/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/043-web-font-size-accessibility-toggle/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/043-web-font-size-accessibility-toggle/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-043): Web Font Size Accessibility Toggle`, do NOT push
...
git commit -m "chore(F-043): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/043-web-font-size-accessibility-toggle/` |
| Context Snapshot | `.prizmkit/specs/043-web-font-size-accessibility-toggle/context-snapshot.md` |
