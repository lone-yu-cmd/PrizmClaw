...
- **Feature ID**: F-044 | **Session**: F-044-20260323163553 | **Run**: run-20260322-013453
...
You are the **session orchestrator**. Implement Feature F-044: "Web Panel Resize Handle".
...
为 Web 管理台聊天区与右侧面板之间添加可拖拽分隔条，允许用户自由调整两列宽度比例。在 public/styles.css 和 public/index.html 中实现拖拽 resize handle，宽度比例持久化至 localStorage。无后端变更。
...
- F-030 - Web Responsive Mobile Layout (completed)
...
```
.prizmkit/specs/044-web-panel-resize-handle/context-snapshot.md
.prizmkit/specs/044-web-panel-resize-handle/plan.md              ← includes Tasks section
```
...
```bash
ls .prizmkit/specs/044-web-panel-resize-handle/context-snapshot.md 2>/dev/null && echo "EXISTS" || echo "MISSING"
```
...
3. Write `.prizmkit/specs/044-web-panel-resize-handle/context-snapshot.md`:
...
```bash
ls .prizmkit/specs/044-web-panel-resize-handle/ 2>/dev/null
```
...
**4b. Commit** — Run `/prizmkit-committer` → `feat(F-044): Web Panel Resize Handle`, do NOT push
...
git commit -m "chore(F-044): include session artifacts"
```
...
| Resource | Path |
|----------|------|
| Feature Artifacts Dir | `.prizmkit/specs/044-web-panel-resize-handle/` |
| Context Snapshot | `.prizmkit/specs/044-web-panel-resize-handle/context-snapshot.md` |
